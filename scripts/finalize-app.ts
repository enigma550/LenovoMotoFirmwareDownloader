...
const identifier: string =
  ...
  "com.github.enigma550.lenovomotofwdl";
...
const pickAppFolder = (): { appFolder: string; appName: string } | null => {
  ...
  const checkFolder = (folderPath: string): boolean => {
    ...
    if (existsSync(join(folderPath, "Contents", "MacOS", launcherName)))
      return true;
    return false;
  };

  const directMatch: string | null = envAppName
    ? join(buildDir, envAppName)
    : null;
  const directMatchApp: string | null = envAppName
    ? join(buildDir, `${envAppName}.app`)
    : null;
  ...
  if (
    directMatchApp &&
    existsSync(directMatchApp) &&
    checkFolder(directMatchApp)
  ) {
  ...
  if (candidates.length === 1) {
    ...
    return {
      appFolder: join(buildDir, name),
      appName: envAppName || name.replace(".app", ""),
    };
  }
...
const patchLinuxWrapper = (appFolder: string): void => {
  ...
  if (classBytes.length > original.length) {
    console.warn(
      `WMClass '${wmClass}' is too long. Truncating to ${original.length} chars.`,
    );
  ...
  const patchedName: string = new TextDecoder().decode(
    classBytes.length > original.length
      ? classBytes.subarray(0, original.length)
      : classBytes,
  );
...
const writeLinuxScripts = (appFolder: string): void => {
...
if (targetOS === "linux") {
  ...
  writeLinuxScripts(app.appFolder);
  ...
  if (!existsSync(iconPath)) {
    ...
    try {
      ...
      console.error(
        `FinalizeApp Error: Failed to copy icon to ${appIcoPath}`,
        err,
      );
    }
    ...
    import("rcedit")
      .then((moduleValue: ModuleLikeValue) => {
        ...
        if (existsSync(launcherPath)) {
          console.log(
            `FinalizeApp: Embedding icon and metadata into launcher -> ${launcherPath}`,
          );
        ...
        if (existsSync(bunExePath)) {
          console.log(
            `FinalizeApp: Embedding icon and metadata into bun -> ${bunExePath}`,
          );
        ...
        if (rceditPromises.length > 0) {
          Promise.all(rceditPromises)
            .then(() => {
              console.log(
                "FinalizeApp: Successfully applied application icons and metadata.",
              );
            })
            .catch((err) => {
              console.error(
                "FinalizeApp Error: Failed to apply icons/metadata!",
                err,
              );
            });
        }
  ...
  if (existsSync(plistPath)) {
    try {
      ...
      console.log(
        `FinalizeApp: Patched Info.plist to use full name '${desktopDisplayName}' for macOS.`,
      );
    } catch (error) {
