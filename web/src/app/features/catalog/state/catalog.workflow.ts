...
import { CatalogDesktopApiService } from '../../../core/api/desktop';
import type {
...
export class CatalogWorkflowService {
  private readonly backend = inject(CatalogDesktopApiService);
  ...
  async runConnectedLookup() {
    await this.ui.runAction('Looking up firmware from connected device...', async () => {
      ...
      const toolText = `adb=${response.adbAvailable ? 'yes' : 'no'}`;
