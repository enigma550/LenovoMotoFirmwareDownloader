...
export class SystemWorkflowService {
  ...
  async installWindowsEdlDriver() {
    try {
      this.ui.status.set('Installing Windows EDL driver...');
      const response = await this.backend.installWindowsEdlDriver();
      if (response.ok) {
        this.ui.status.set('Windows EDL driver install completed.');
        this.ui.showToast(
          response.detail || 'Windows EDL driver install completed.',
          'success',
          3200,
        );
      } else {
        const message = response.error || response.detail || 'Windows EDL driver install failed.';
        this.ui.status.set('Windows EDL driver install failed.');
