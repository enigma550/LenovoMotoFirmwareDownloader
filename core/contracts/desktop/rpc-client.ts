...
export function createDesktopRpcClient(options: {
  ...
  function postRpcRequest(method: string, params?: RpcBridgePayload) {
    ...
    return new Promise<RpcBridgePayload | undefined>((resolve, reject) => {
      ...
      try {
        const maybePromise = bridge.postMessage(
          JSON.stringify({
            type: 'request',
            id: requestId,
            method,
        ...
        if (
          ...
          void (maybePromise as Promise<unknown>).catch((error) => {
            ...
            pending.reject(new Error(`Bridge postMessage failed for ${method}: ${errorMessage}`));
          });
        }
      } catch (error) {
        ...
        pendingRequests.delete(requestId);
        const errorMessage = error instanceof Error ? error.message : String(error);
        reject(new Error(`Bridge postMessage threw for ${method}: ${errorMessage}`));
      }
    });
  }
