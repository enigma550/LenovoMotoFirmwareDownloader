...
async function isProjectRoot(dir: string) {
  ...
  if (
    !(await Bun.file(packageJsonPath).exists()) ||
    !(await Bun.file(electrobunConfigPath).exists())
  ) {
    return false;
  }
...
const runtimeUserDataDir = runtimeVersionInfo
  ? join(getAppDataDir(), runtimeVersionInfo.identifier, runtimeVersionInfo.channel)
  : null;
const shouldUseRuntimeUserData = runtimeVersionInfo
  ? runtimeVersionInfo.channel !== 'dev'
  : !projectRootLookup.found;
const shouldUseProjectData = projectRootLookup.found && !shouldUseRuntimeUserData;
...
const LEGACY_CONFIG_PATH = shouldUseProjectData ? resolve(PROJECT_ROOT, 'config.json') : null;
