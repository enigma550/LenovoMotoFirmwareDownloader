...
type LayerName = 'core' | 'runtime' | 'web' | 'tooling';
...
const BOUNDARY_RULES: BoundaryRule[] = [
  { source: 'core', forbiddenTargets: ['runtime', 'web', 'tooling'] },
  { source: 'runtime', forbiddenTargets: ['web', 'tooling'] },
  { source: 'web', forbiddenTargets: ['runtime', 'tooling'] },
  { source: 'tooling', forbiddenTargets: [] },
];
...
function detectLayerFromPath(filePath: string): LayerName | null {
  ...
  if (normalizedPath.includes('/tooling/')) return 'tooling';
...
function isBareSpecifier(importSpecifier: string) {
  return !isRelativeSpecifier(importSpecifier) && !importSpecifier.startsWith('/');
}

async function resolveRelativeImport(fromFilePath: string, importSpecifier: string) {
...
async function detectTargetLayer(
  ...
  if (isRelativeSpecifier(importSpecifier)) {
    const resolvedPath = await resolveRelativeImport(fromFilePath, importSpecifier);
  ...
  if (isBareSpecifier(importSpecifier)) {
    ...
    if (importSpecifier.startsWith('tooling/')) return 'tooling';
...
function isForbiddenImport(sourceLayer: LayerName, targetLayer: LayerName) {
  const rule = BOUNDARY_RULES.find((candidateRule) => candidateRule.source === sourceLayer);
  return Boolean(rule?.forbiddenTargets.includes(targetLayer));
}
