...
const pausedDownloadPayloads = new Map<string, DownloadTaskPayload>();
...
function inferFileName(romUrl: string, romName: string, fromHeader: string) {
  ...
  const chosenName = fromHeader || urlFileName || `${romName || 'firmware'}.zip`;
...
export async function downloadFirmwareWithProgress(
  ...
  let savePath = '';
  let downloadedBytes = payload.downloadedBytes || 0;
  let totalBytes: number | undefined = payload.totalBytes;
  ...
  try {
    ...
    const optimisticPath = getDownloadPath(downloadDirectory, optimisticFileName);
    if (downloadedBytes === 0 && (await Bun.file(optimisticPath).exists())) {
      return {
        ok: false,
        downloadId,
        error: `Download skipped. Archive already exists: ${optimisticPath}`,
      };
    }
    ...
    const totalFromRange = contentRangeMatch?.[1];
    if (totalFromRange) {
      totalBytes = Number.parseInt(totalFromRange, 10);
    }
    ...
    if (downloadedBytes === 0 && (await Bun.file(finalPath).exists())) {
      return {
        ok: false,
        downloadId,
        error: `Download skipped. Archive already exists: ${finalPath}`,
      };
    }
    savePath = finalPath;
      ...
      if (!response.body) {
        ...
        while (true) {
          ...
          if (elapsedLastSnapshot >= 0.8) {
            ...
            const speedBytesPerSecond = bytesSinceLastSnapshot / elapsedLastSnapshot;
    ...
    const elapsedTotalSeconds = Math.max((completedAt - startedAt) / 1000, 0.001);
...
export function cancelActiveDownload(downloadId: string): CancelDownloadResponse {
...
export function pauseActiveDownload(downloadId: string): {
  ok: boolean;
  error?: string;
} {
  const activeDownload = activeDownloads.get(downloadId);
