...
> {
  return {
    ...
    lookupConnectedDeviceFirmwareFromDeviceInfo: async ({ device, adbAvailable }) => {
      try {
        return await lookupConnectedDeviceFirmwareFromDeviceInfo(device, {
          adbAvailable,
        });
      } catch (error) {
        return {
          ok: false,
          adbAvailable: Boolean(adbAvailable),
          attempts: [],
          variants: [],
          error: toErrorMessage(error),
        } satisfies ConnectedLookupResponse;
