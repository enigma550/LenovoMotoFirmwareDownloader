...
function readSubprocessStreamText(stream: Bun.Subprocess['stdout'] | Bun.Subprocess['stderr']) {
...
async function runExtractionStrategy(
  ...
  try {
    const [exitCode, stderr, stdout] = await Promise.all([
      processRef.exited,
      readSubprocessStreamText(processRef.stderr),
      readSubprocessStreamText(processRef.stdout),
    ]);
