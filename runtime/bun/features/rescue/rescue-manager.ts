...
import { ensureWindowsEdlDriver } from './commands/wdi-driver-installer.ts';
import {
  hasFastbootDevice,
  isCommandAvailable,
  probeQualcommEdlUsb,
  ...
  try {
    ...
    if (hasEdlCommands) {
      ...
      if (process.platform === 'win32') {
        ...
        const driverEnsureResult = await ensureWindowsEdlDriver({
          cwd: workDir,
          signal: rescueController.signal,
          onProcess: setActiveProcess,
        });
    ...
    if (hasUnisocCommands) {
      ...
      if (!resolvedTool) {
        throw new Error(
          `Unisoc PAC rescue requires a supported tool in PATH (${unisocToolCandidates.join(', ')}). ` +
