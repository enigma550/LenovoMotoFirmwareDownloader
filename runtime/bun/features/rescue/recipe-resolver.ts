...
export async function resolveRescueRecipeHints(
  ...
  let recipeContent: JsonValue | undefined;
  let source = '';
  ...
  if (directRecipeUrl) {
    ...
    const modelName =
      selectedParameters.modelName || selectedParameters.modelCode || selectedParameters.sku || '';
    const marketName = selectedParameters.marketName || '';
    const category = selectedParameters.category || '';

    if (modelName || marketName) {
      const recipeInfoResponse = await requestApi('/rescueDevice/getRescueModelRecipe.jhtml', {
        modelName,
        marketName,
        category,
      });
      const recipeInfoPayload = asRecord(await recipeInfoResponse.json());
      const code = typeof recipeInfoPayload?.code === 'string' ? recipeInfoPayload.code : '';
      if (code === '0000') {
        ...
        } else if (Array.isArray(content?.Steps) || Array.isArray(content?.steps)) {
  ...
  for (const reference of references) {
    mapRecipeReferenceToPreferredFileNames(reference, dataReset, preferredFileNames);
  }
...
function collectRecipeReferences(recipeContent: JsonValue) {
