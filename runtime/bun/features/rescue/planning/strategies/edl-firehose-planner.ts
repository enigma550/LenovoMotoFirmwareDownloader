...
function findFirehoseProgrammer(extractedFiles: string[], preferredProgrammerPaths: string[] = []) {
...
function pickPatchXml(extractedFiles: string[]) {
  ...
  const patch0 = candidates.find((candidate) => basename(candidate).toLowerCase() === 'patch0.xml');
  return patch0 || candidates[0] || '';
}
...
export const edlFirehosePlannerStrategy: RescueCommandPlannerStrategy = {
  ...
  async plan(context) {
    ...
    const patchPath = pickPatchXml(context.extractedFiles);
    ...
    if (!programmerPath) {
      programmerPath = findFirehoseProgrammer(allExtractedFiles, preferredProgrammerPaths);
    }
    if (!programmerPath) {
    ...
    try {
      ...
      // Ignore parsing hint failures and fall back to filename/loadinfo heuristics.
    }
