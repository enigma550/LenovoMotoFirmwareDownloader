import {
  disposeRescueCommandExecutionContext,
  runPreparedRescueCommand,
} from './rescue-command-runner-registry.ts';
...
export async function runRescueCommandPlan(options: {
  ...
  const executionContext = {
    ...
    onConsoleLine: options.onConsoleLine,
    state: {
      fastbootClient: undefined,
      fastbootReconnectMatcher: undefined,
  ...
  try {
    ...
    await disposeRescueCommandExecutionContext(executionContext);
  }
}
