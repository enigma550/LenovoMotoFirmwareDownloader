import type { DesktopApi } from "../../core/shared/types/desktop-api.ts";
...
export function createDesktopApiFromInvoker(
  invokeRpc: DesktopRpcInvoker,
): DesktopApi {
  return {
    isDesktop: true,
    startAuth: () =>
      invokeRpc("authStart") as ReturnType<DesktopApi["startAuth"]>,
    completeAuth: (callbackUrlOrToken) =>
      invokeRpc("authComplete", { callbackUrlOrToken }) as ReturnType<
        DesktopApi["completeAuth"]
      >,
    getStoredAuthState: () =>
      invokeRpc("getStoredAuthState") as ReturnType<
        DesktopApi["getStoredAuthState"]
      >,
    authWithStoredToken: () =>
      invokeRpc("authWithStoredToken") as ReturnType<
        DesktopApi["authWithStoredToken"]
      >,
    ping: () => invokeRpc("ping") as ReturnType<DesktopApi["ping"]>,
    getCatalogModels: (refresh = false) =>
      invokeRpc("getCatalogModels", { refresh }) as ReturnType<
        DesktopApi["getCatalogModels"]
      >,
    ...
    downloadFirmware: (payload) =>
      invokeRpc("downloadFirmware", payload) as ReturnType<
        DesktopApi["downloadFirmware"]
      >,
    rescueLiteFirmware: (payload) =>
      invokeRpc("rescueLiteFirmware", payload) as ReturnType<
        DesktopApi["rescueLiteFirmware"]
      >,
    ...
    extractLocalFirmware: (payload) =>
      invokeRpc("extractLocalFirmware", payload) as ReturnType<
        DesktopApi["extractLocalFirmware"]
      >,
    ...
    cancelDownload: (payload) =>
      invokeRpc("cancelDownload", payload) as ReturnType<
        DesktopApi["cancelDownload"]
      >,
    checkDesktopIntegration: () =>
      invokeRpc("checkDesktopIntegration") as ReturnType<
        DesktopApi["checkDesktopIntegration"]
      >,
    createDesktopIntegration: () =>
      invokeRpc("createDesktopIntegration") as ReturnType<
    ...
    setDesktopPromptPreference: (payload) =>
      ...
      >,
    getAppInfo: () =>
      invokeRpc("getAppInfo") as ReturnType<
        DesktopApi["getAppInfo"]
      >,
    listLocalDownloadedFiles: () =>
      invokeRpc("listLocalDownloadedFiles") as ReturnType<
        DesktopApi["listLocalDownloadedFiles"]
      >,
    openUrl: (url) =>
      invokeRpc("openUrl", { url }) as ReturnType<DesktopApi["openUrl"]>,
    checkFrameworkUpdate: () =>
      invokeRpc("checkFrameworkUpdate") as ReturnType<
        DesktopApi["checkFrameworkUpdate"]
      >,
    downloadFrameworkUpdate: () =>
      invokeRpc("downloadFrameworkUpdate") as ReturnType<
        DesktopApi["downloadFrameworkUpdate"]
      >,
    applyFrameworkUpdate: () =>
      invokeRpc("applyFrameworkUpdate") as ReturnType<
        DesktopApi["applyFrameworkUpdate"]
      >,
    deleteLocalFile: (payload) =>
      invokeRpc("deleteLocalFile", payload) as ReturnType<
        DesktopApi["deleteLocalFile"]
      >,
    pauseDownload: (payload) =>
      invokeRpc("pauseDownload", payload) as ReturnType<
        DesktopApi["pauseDownload"]
      >,
    resumeDownload: (payload) =>
      invokeRpc("resumeDownload", payload) as ReturnType<
        DesktopApi["resumeDownload"]
      >,
  };
}
